...
interface JQuery {
    isInView(options?: isInViewOptions): Object
}

(($)=> {
    $.fn.isInView = function (this: JQuery, options?: isInViewOptions) {

        /* Default options: Fully or Partially, OffsetTop. */
        let settings: isInViewOptions = $.extend({
        ...
        let docViewTop:number = $(window).scrollTop() + settings.offsetTop;
        let docHeight:number = $(window).height() - settings.offsetTop;
        let docViewBottom:number = docViewTop + docHeight;

        /* Element Specs: Top, Bottom, Height */
        let elemTop:number = $(this).offset().top;
        let elemHeight:number =  $(this)[0].offsetHeight;
        let elemBottom:number = elemTop + elemHeight;
        // var elemBottom = elemTop + $(this).height();

        /* The distance has been passed by the element in the viewport*/
        let passed:number = (docViewTop - elemTop + docHeight);

        /* The distance element should pass when it will cross the whole viewport */
        let distanceToPass:number = (docHeight + elemHeight);
